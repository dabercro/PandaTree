...
panda::Jet::getListOfBranches()
{
  ...
  blist += {"area", "bRegCorr", "bRegRes", "btagCMVA", "btagCSVV2", "btagDeepB", "btagDeepC", "btagDeepFlavB", "chEmEF", "chHEF", "muEF", "neEmEF", "neHEF", "qgl", "rawFactor", "jetId", "nConstituents", "nElectrons", "nMuons", "puId", "electronIdx1_", "electronIdx2_", "muonIdx1_", "muonIdx2_"};
  return blist;
}
...
panda::Jet::datastore::allocate(UInt_t _nmax)
{
  ...
  electronIdx1_ = new Short_t[nmax_];
  electronIdx2_ = new Short_t[nmax_];
  muonIdx1_ = new Short_t[nmax_];
  muonIdx2_ = new Short_t[nmax_];
}
...
panda::Jet::datastore::deallocate()
{
  ...
  delete [] puId;
  puId = 0;
  delete [] electronIdx1_;
  electronIdx1_ = 0;
  delete [] electronIdx2_;
  electronIdx2_ = 0;
  delete [] muonIdx1_;
  muonIdx1_ = 0;
  delete [] muonIdx2_;
...
panda::Jet::datastore::setStatus(TTree& _tree, TString const& _name, utils::BranchList const& _branches)
{
  ...
  utils::setStatus(_tree, _name, "electronIdx1_", _branches);
  utils::setStatus(_tree, _name, "electronIdx2_", _branches);
  utils::setStatus(_tree, _name, "muonIdx1_", _branches);
  utils::setStatus(_tree, _name, "muonIdx2_", _branches);
}
...
panda::Jet::datastore::getStatus(TTree& _tree, TString const& _name) const
{
  ...
  blist.push_back(utils::getStatus(_tree, _name, "electronIdx1_"));
  blist.push_back(utils::getStatus(_tree, _name, "electronIdx2_"));
  blist.push_back(utils::getStatus(_tree, _name, "muonIdx1_"));
  blist.push_back(utils::getStatus(_tree, _name, "muonIdx2_"));
...
panda::Jet::datastore::setAddress(TTree& _tree, TString const& _name, utils::BranchList const& _branches/* = {"*"}*/, Bool_t _setStatus/* = kTRUE*/)
{
  ...
  utils::setAddress(_tree, _name, "electronIdx1_", electronIdx1_, _branches, _setStatus);
  utils::setAddress(_tree, _name, "electronIdx2_", electronIdx2_, _branches, _setStatus);
  utils::setAddress(_tree, _name, "muonIdx1_", muonIdx1_, _branches, _setStatus);
  utils::setAddress(_tree, _name, "muonIdx2_", muonIdx2_, _branches, _setStatus);
}
...
panda::Jet::datastore::book(TTree& _tree, TString const& _name, utils::BranchList const& _branches/* = {"*"}*/, Bool_t _dynamic/* = kTRUE*/)
{
  ...
  utils::book(_tree, _name, "electronIdx1_", size, 'S', electronIdx1_, _branches);
  utils::book(_tree, _name, "electronIdx2_", size, 'S', electronIdx2_, _branches);
  utils::book(_tree, _name, "muonIdx1_", size, 'S', muonIdx1_, _branches);
  utils::book(_tree, _name, "muonIdx2_", size, 'S', muonIdx2_, _branches);
}
...
panda::Jet::datastore::releaseTree(TTree& _tree, TString const& _name)
{
  ...
  utils::resetAddress(_tree, _name, "electronIdx1_");
  utils::resetAddress(_tree, _name, "electronIdx2_");
  utils::resetAddress(_tree, _name, "muonIdx1_");
  utils::resetAddress(_tree, _name, "muonIdx2_");
}
...
panda::Jet::Jet(char const* _name/* = ""*/) :
  ...
  electronIdx1(gStore.getData(this).electronIdx1Container_, gStore.getData(this).electronIdx1_[0]),
  electronIdx2(gStore.getData(this).electronIdx2Container_, gStore.getData(this).electronIdx2_[0]),
  muonIdx1(gStore.getData(this).muonIdx1Container_, gStore.getData(this).muonIdx1_[0]),
  muonIdx2(gStore.getData(this).muonIdx2Container_, gStore.getData(this).muonIdx2_[0])
{
}
...
panda::Jet::Jet(datastore& _data, UInt_t _idx) :
  ...
  electronIdx1(_data.electronIdx1Container_, _data.electronIdx1_[_idx]),
  electronIdx2(_data.electronIdx2Container_, _data.electronIdx2_[_idx]),
  muonIdx1(_data.muonIdx1Container_, _data.muonIdx1_[_idx]),
  muonIdx2(_data.muonIdx2Container_, _data.muonIdx2_[_idx])
{
}
...
panda::Jet::doBook_(TTree& _tree, TString const& _name, utils::BranchList const& _branches/* = {"*"}*/)
{
  ...
  utils::book(_tree, _name, "electronIdx1_", "", 'S', gStore.getData(this).electronIdx1_, _branches);
  utils::book(_tree, _name, "electronIdx2_", "", 'S', gStore.getData(this).electronIdx2_, _branches);
  utils::book(_tree, _name, "muonIdx1_", "", 'S', gStore.getData(this).muonIdx1_, _branches);
  utils::book(_tree, _name, "muonIdx2_", "", 'S', gStore.getData(this).muonIdx2_, _branches);
}
